...
export const DatabaseViewer: React.FC<DatabaseViewerProps> = ({ onClose }) => {
  const { triggerToast } = useApp();
  const [isAdminUnlocked, setIsAdminUnlocked] = useState<boolean>(() => {
    return sessionStorage.getItem('exportready_admin_unlocked') === 'true';
  });
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Default master admin pin: admin2026 or exportready
    if (adminPin === 'admin2026' || adminPin === 'exportready' || adminPin === 'admin') {
      setIsAdminUnlocked(true);
      sessionStorage.setItem('exportready_admin_unlocked', 'true');
      setPinError('');
      triggerToast('Admin Authorized', 'Master database administration console unlocked.', 'success');
    } else {
  ...
  const handleAdminLock = () => {
    ...
    sessionStorage.removeItem('exportready_admin_unlocked');
    setAdminPin('');
    triggerToast('Admin Session Locked', 'Database viewer is now locked.', 'info');
  };
  ...
  if (!isAdminUnlocked) {
    return (
      <div className="max-w-md mx-auto my-12 p-8 glass-card rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl text-center animate-in zoom-in-95">
        <div className="w-14 h-14 rounded-2xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center mx-auto mb-4 border border-purple-500/20">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          This section contains raw database tables and multi-user accounts. Please enter your administrator passcode.
        </p>
        ...
        <form onSubmit={handleAdminLogin} className="mt-6 space-y-4">
          <div>
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl text-center text-sm font-mono tracking-widest bg-slate-50 dark:bg-navy-900 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white focus:outline-none focus:border-purple-500"
              autoFocus
            />
            {pinError && (
          ...
          <div className="flex gap-2">
            {onClose && (
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-navy-800 hover:bg-slate-200 transition-colors"
              >
                Cancel
              </button>
            )}
            <button
              type="submit"
              className="flex-1 py-2.5 rounded-xl text-xs font-bold text-white bg-purple-600 hover:bg-purple-500 shadow-md transition-all cursor-pointer"
            >
              Verify & Unlock Database
            </button>
          </div>
        </form>

        <p className="text-[10px] text-slate-400 mt-4">
          Default Passcode: <code className="font-mono text-purple-600 dark:text-purple-400">admin2026</code>
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 border border-rose-300 dark:border-rose-800 flex items-center gap-1.5 transition-colors cursor-pointer"
          >
